...
// as in dev (better-sqlite3 is native and *must* stay external; the AI SDK
// packages and @modelcontextprotocol/sdk are kept external to avoid inlining
// their dynamic requires). Versions are read from the workspace package.json
// that actually declares each dep, so the released package can never drift
// from what dev resolves.
