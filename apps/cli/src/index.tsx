...
import { createTurnRenderer, type TurnFinish } from "./turn-events";
import type { Item } from "./types";
import { ItemView } from "./ui/ItemView";
import { runLaunchPicker } from "./ui/LaunchPicker";
import { runSetup } from "./ui/Setup";
import { LAPIS, SAGE, SAND, STONE } from "./ui/theme";

// Retrieval tier the `researcher` subagent runs on — a registry id. When
// unset, the runtime falls back to the registry's "research"-tagged entry.
const RESEARCH_MODEL = process.env.HEMIUNU_MODEL_RESEARCH;
...
// The lookup is the model registry entry — the same number the engine's
// compactor thresholds against.
const ENV_WINDOW = process.env.HEMIUNU_CONTEXT_WINDOW
  ? Number(process.env.HEMIUNU_CONTEXT_WINDOW)
  : undefined;
function contextWindowFor(model: string, models: ModelEntry[]): number {
  return ENV_WINDOW ?? models.find((m) => m.id === model)?.contextWindow ?? 200_000;
}
...
function App({
  ...
  /** The engine runtime — one per process (transcript, compactor, MCP host). */
  runtime: EngineRuntime;
  /** The model registry entries (what /models lists and ids resolve against). */
  models: ModelEntry[];
  systemPrompt: string;
  initialModel: string;
  ...
  // Streamed model reasoning — shown dim while the model thinks, never committed.
  ...
  const ctxWindow = contextWindowFor(model, models);

  // Only expose the filesystem server once the user has trusted this folder.
  // Memoized: this component re-renders on every stream tick, and rebuilding
  // the filtered registry each time churns objects for a value that only
  // changes when trust flips.
  ...
  const activeServers = React.useMemo(
    ...
    [fsOn, fsName, registry.mcpServers],
  );
  const activePatterns = React.useMemo(
    ...
    [fsOn, fsName, registry.toolPatterns],
  );
  ...
  // The plan the exit_plan_mode gate just rendered — so the plan-proposed
  // event doesn't print it twice (it still prints on auto-accepted plans).
  const planShownRef = useRef("");
  ...
  const effectiveSystem = () => {
    ...
    const sourceMaps = maps.length
      ...
      : "";
    // NB: no compacted-summary block anymore — the engine transcript carries
    // the latest compaction summary itself and folds it into every resume.
    return systemPrompt + sources + skillList + sourceMaps;
  };

  // Interactive permission gate for the engine's pipeline. Prompts are already
  // serialized pipeline-side, and the persistent tool policy (allow/block),
  // the session always-allow Set, and the auto-approved tools (remember,
  // source maps, ask_user, todo/plan control tools) are all enforced BEFORE
  // this gate — the engine only asks about genuinely undecided tools.
  const canUseTool = (toolName: string, input: unknown): Promise<CanUseToolResult> =>
    new Promise((resolve) => {
      // exit_plan_mode: the plan-approval gate. Render the plan, then show the
      // three-way Claude-Code menu (auto-accept / manual / keep planning).
      if (toolName === PLAN_EXIT_TOOL) {
        const plan =
          typeof input === "object" && input !== null
            ? String((input as Record<string, unknown>).plan ?? "")
            : "";
        if (plan) {
          planShownRef.current = plan; // plan-proposed needn't re-print it
          push({ kind: "note", text: `Proposed plan:\n${plan}` });
        }
        flushGroup();
        setSel(0);
        setPermission({
          ...
          onChoice: (choice) => {
            ...
            if (choice === "plan-refine") {
              ...
              resolve({ behavior: "deny", message: PLAN_REFINE_MESSAGE });
              return;
            }
            // Accepted: an allow lets exit_plan_mode.execute() run, which is
            // what flips the loop back out of read-only plan mode. "auto"
            // turns on auto-accept (every following tool runs without a
            // prompt — per this team); "manual" approves each step.
            setPlanMode(false);
      ...
      // Auto-accept mode: approve every gated tool without a prompt. The ref
      // is current even within the turn a plan was just approved in.
      ...
      setPermission({
        ...
        onChoice: (choice) => {
          setPermission(null);
          if (choice === "always") {
            alwaysAllow.current.add(toolName); // immediate (the pipeline reads the Set)
  ...
  async function runUserTurn(text: string) {
    ...
    if (currentProjectRef.current && !(await ensureTeamAlive(currentProjectRef.current)))
      switchProject(null);
    const newConversation = !sessionId.current;
    // Holder object: the value is assigned inside a sink callback, which TS's
    // control-flow analysis would otherwise narrow away after the loop.
    const turn: { finish: TurnFinish | null } = { finish: null };
    ...
    // Commit the live streamed answer to scrollback (a tool call or the turn's
    // end interrupts prose).
    const flushText = () => {
    ...
    // The TurnEvent → UI mapping (turn-events.ts), wired to this component.
    const renderer = createTurnRenderer({
      push,
      appendText: (t) => {
        // Top-level prose resumes — close any open activity group first so it
        // lands in scrollback above the answer.
        if (!liveRef.current && t.trim()) flushGroup();
        clearThinking();
        liveRef.current += t;
        turnTokensRef.current += Math.ceil(t.length / 4);
      ...
      feedActivity,
      flushGroup,
      // Fold the result into the live group as its latest detail (a top-level
      // tool-run shows the result inline), not a separate line.
      foldResult: (summary) => {
        const g = groupRef.current;
      ...
      // The engine's conversation id IS the session id (resume key).
      onConversation: (id) => {
        sessionId.current = id;
      },
      onPlan: (plan) => {
        // Shown by the exit_plan_mode gate already — only print here when the
        // plan was auto-approved (auto-accept mode skips the gate).
        if (planShownRef.current !== plan) push({ kind: "note", text: `Proposed plan:\n${plan}` });
        planShownRef.current = "";
      },
      // A step's input-side tokens ARE the current context size.
      onStep: (u) => setCtx(u.inputTokens + u.cacheReadTokens + u.cacheWriteTokens),
      onFinish: (f) => {
        turn.finish = f;
      },
    });
    ...
    try {
      for await (const e of runtime.runTurn({
        prompt: text,
        model,
        ...(RESEARCH_MODEL ? { researchModel: RESEARCH_MODEL } : {}),
        ...
        // (No team → no workspace binding; tools fall back to the local session.)
        ...(currentProjectRef.current ? { workspace: { repo: currentProjectRef.current } } : {}),
        alwaysAllow: alwaysAllow.current,
      })) {
        renderer.handle(e);
      }
      flushGroup();
    ...
    if (done) {
      if (done.stopReason === "aborted") push({ kind: "note", text: "⎯ interrupted" });
      else if (done.stopReason === "max-steps")
        push({ kind: "note", text: "· stopped at the step limit — say “continue” to keep going" });
      // Cost comes from the engine (registry pricing) — no CLI-side price table.
      push({ kind: "cost", text: `$${done.costUsd.toFixed(4)} · ${done.usage.outputTokens} out` });
      setSessionCost((c) => c + done.costUsd);
    }
    ...
    if (sid) {
      ...
      store.addMessage(sid, "assistant", done?.text ?? "", done?.costUsd ?? null);
      // A fresh conversation gets an engine-generated title (the registry's
      // title-tagged model) — fire-and-forget; the clipped first message above
      // stays as the fallback when generation fails (offline, no key, …).
      if (newConversation) {
        void runtime
          .generateTitle(text)
          .then((t) => {
            if (t) store.setTitle(sid, t);
          })
          .catch(() => {});
      }
    }
    ...
    // NB: no CLI-side auto-compaction anymore — the engine's Compactor watches
    // the context inside every turn and folds the transcript when it crosses
    // the threshold (surfaced as the `compaction` TurnEvent).
  }
  ...
  async function reconcilePrototype({
    ...
    const res = await askAnthropic({
      // askAnthropic talks to the Anthropic API directly (the brain's own
      // key/endpoint) — keep the old retrieval-tier default when no override.
  ...
  // /compact — a thin call to the engine's compactor: it summarizes the live
  // transcript, records the compaction, and every later resume of the SAME
  // conversation id starts from the summary (no session reset, no CLI-side
  // summary bookkeeping). Auto-compaction is the engine's job now (the
  // Compactor runs inside every turn and emits the `compaction` event).
  async function runCompact() {
    const sid = sessionId.current;
    if (!sid) {
      push({ kind: "note", text: "· nothing to compact yet — the conversation is empty" });
      return;
    }
    turnStartRef.current = Date.now();
    turnTokensRef.current = 0;
    ...
    try {
      const summary = (await runtime.compactNow(sid, model)).trim();
      setCtx(0);
      setEpoch((e) => e + 1);
  ...
  // /models — the model picker over the REGISTRY (shipped defaults merged with
  // ~/.hemiunu/models.json): every row shows id, label, provider, context
  // window, and role tags, with the active entry marked. Selecting persists
  // HEMIUNU_MODEL (same as before); the conversation continues — the engine
  // transcript is provider-neutral, so no session reset is needed.
  function showModels() {
    const options = models.map((m) => modelRow(m, model));
    setSel(
      Math.max(
        0,
        models.findIndex((m) => m.id === model),
      ),
    );
    setPicker({
      title:
        "Select model — registry ids; add your own in ~/.hemiunu/models.json  (↑/↓ · Enter · Esc)",
      options,
      onChoice: (v) => {
        setPicker(null);
        if (v === null) return;
        const picked = models[options.indexOf(v)];
        if (!picked || picked.id === model) return;
        setModel(picked.id);
        upsertUserEnv("HEMIUNU_MODEL", picked.id); // remember it for next launch
        push({
          kind: "note",
          text: `· model set to ${picked.id} — ${picked.label} (saved). The conversation continues on it.`,
        });
      },
    });
  }
  ...
  async function startGithubLogin() {
    ...
    openExternal(dc.verificationUri);
  ...
  function handleCommand(text: string) {
    ...
    if (cmd === "models") return showModels();
    if (cmd === "new") {
    ...
    if (cmd === "setup") {
      // Provider-agnostic readout: one line per key env the model registry
      // references — any ONE of them (or a keyless local endpoint) is enough.
      const set = (v?: string) => (v && v.trim() ? "✓" : "✗");
      const envs = [...new Set(models.map((m) => keyEnvFor(m)).filter((e): e is string => !!e))];
      return push({
        kind: "note",
        text:
          `config: ${join(configDir(), ".env")}\n` +
          `${envs.map((e) => `  ${e} ${set(process.env[e])}`).join("\n")}\n` +
          `any one key unlocks its models (local Ollama needs none).\n` +
    ...
    if (cmd === "settings") {
      ...
      return push({
        ...
        text:
          ...
          `  research model   ${RESEARCH_MODEL ?? modelForTag("research", models, model).id}   → HEMIUNU_MODEL_RESEARCH in .env\n` +
          ...
          `  keys             ${
            [...new Set(models.map((m) => keyEnvFor(m)).filter((e): e is string => !!e))]
              .filter((e) => !!process.env[e]?.trim())
              .map((e) => e.replace(/_API_KEY$/, ""))
              .join(", ") || "none set"
          }   → /setup\n` +
    ...
    if (cmd === "resume") {
      ...
      // The engine transcript is the source of truth for resume: the
      // conversation id IS the session id, and the next turn replays the
      // stored history (plus any compaction summary) from it.
      const msgs = runtime.transcript.load(id).messages;
  ...
  return (
    <Box flexDirection="column">
      ...
      {/* Streamed model reasoning — dim, live-only; discarded once real output
          (answer text or a tool call) starts. */}
      {busy && thinking && !live ? (
        <Box marginTop={1}>
...
async function main() {
  ...
  // First run: if NO registry model is usable (no provider/gateway key set,
  // no keyless local endpoint answering), run the provider-agnostic setup —
  // pick a provider, paste only THAT key (or a gateway URL+key, or point at a
  // local Ollama) — and write ~/.hemiunu/.env. No env var is special-cased.
  if (!(await registryReady(loadModelRegistry()))) await runSetup();
  ...
  // Model ids resolve through the registry (shipped defaults merged with
  // ~/.hemiunu/models.json — reloaded here because first-run setup may have
  // just registered gateway models): honor a saved HEMIUNU_MODEL when it names
  // a USABLE entry, else start on the first available model — so a LiteLLM-,
  // OpenAI- or Ollama-only machine starts on a model that can actually answer
  // instead of erroring on a Claude id with no Anthropic key.
  const models = loadModelRegistry();
  const model = resolveDefaultModel(models, process.env, process.env.HEMIUNU_MODEL);
  // The engine runtime — built once per process: transcript store (same
  // hemiunu.db), compactor, MCP host (with OAuth headers), model registry.
  const runtime = createEngineRuntime({ registry: models, mcpServers: registry.mcpServers });
