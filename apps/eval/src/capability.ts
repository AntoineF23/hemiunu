...
const MODEL = process.env.HEMIUNU_EVAL_MODEL ?? real.model;
...
function setupSandbox() {
  // Seed the sandbox .env with the real credentials so loadConfig() inside
  // runTurn still authenticates, while all writes go under SANDBOX.
  const env = [
...
async function main() {
  ...
  if (want("S6"))
    await check("S6 produces a self-contained low-fi wireframe", async () => {
      ...
      const v = await judge<{ grayscale: boolean; lowfi: boolean; score: number }>({
        model: JUDGE_MODEL,
        rubric:
  ...
  // (Opus) model; a failure under HEMIUNU_EVAL_MODEL=sonnet is that known gap,
  // not a regression.
  if (want("S8"))
