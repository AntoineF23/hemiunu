...
import { API_KEYS_SECTION, missingKeyFor } from "./model-picker";
...
export function App() {
  const { settings, refresh, setModel, setResearchModel } = useSettings();
  const {
    items,
    busy,
    permission,
  ...
  // "This model needs OPENAI_API_KEY" recovery: the error card opens Settings
  // focused on that key's input. The pickers' "＋ Add API keys…" item lands on
  // the API-keys section too — on the selected model's missing env when there
  // is one, else on the section itself (API_KEYS_SECTION).
  const [keyFocus, setKeyFocus] = useState<string | null>(null);
  const openKeySettings = useCallback((env: string) => {
    setKeyFocus(env);
    setRailAnimate(false);
    setPanel("settings");
    setCollapsed(true);
  }, []);
  // The memory view's agent pickers: straight to the API-keys section.
  const openApiKeys = useCallback(() => openKeySettings(API_KEYS_SECTION), [openKeySettings]);
  // The composer's picker: when the ACTIVE model's key is the one missing,
  // land on that exact input; otherwise on the section.
  const openComposerKeys = useCallback(() => {
    openKeySettings(missingKeyFor(settings?.model, settings?.models ?? []) ?? API_KEYS_SECTION);
  }, [openKeySettings, settings?.model, settings?.models]);
  ...
  const composer = (
    <>
      {/* First-run gate: until at least one registry model is usable (ANY
          provider key, a gateway, or a local Ollama — nothing is special-cased)
          the first turn would fail with a raw error, so offer the setup right
          where the user is about to type. Clears live on the first save. */}
      {settings && !settings.ready && <FirstRunSetup settings={settings} onChanged={refresh} />}
      {reconcilePrompt}
      <Composer
        draft={draft}
        setDraft={setDraft}
    ...
    <div className="relative z-[1] flex h-full">
      ...
      <main className="paper-main relative flex min-w-0 flex-1 flex-col overflow-hidden">
        {panel === "memory" ? (
          ...
          >
            <MemoryView models={settings?.models} onAddKeys={openApiKeys} />
          </Suspense>
        ) : empty ? (
    ...
    <div className={`activity-group${isDelegation ? " delegate" : ""}`}>
      ...
      {open && (
        <div className="activity-children">
          {g.children.map((c, i) => (
            <div key={`${c.label}::${c.preview ?? ""}::${i}`} className="activity sub">
...
/** The engine's "Model X needs OPENAI_API_KEY — add it to ~/.hemiunu/.env"
 *  (resolveModel) — the one raw error a missing key produces at turn start.
 *  Returns the env var name so the UI can steer to Settings → API keys. */
function missingKeyEnv(text: string): string | null {
  return /needs ([A-Z][A-Z0-9_]+) — add it to ~\/\.hemiunu\/\.env/.exec(text)?.[1] ?? null;
}
...
function Item({
  ...
  switch (item.kind) {
    ...
    case "error": {
      // A missing API key isn't a failure the user should read as a crash —
      // it's one Settings field away from working. Point straight at it.
      const env = missingKeyEnv(item.text);
      if (env) {
        return (
          <div className="error">
            <KeyRound size={15} />
            <span>
              The selected model needs an API key ({env}) before it can answer.{" "}
              <button type="button" className="underline" onClick={() => onNeedsKey(env)}>
