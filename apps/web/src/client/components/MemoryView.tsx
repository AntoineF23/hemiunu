...
import { ADD_KEYS_LABEL, ADD_KEYS_VALUE, pickableModels, pickerLabel } from "@/model-picker";
...
/** Native select for a custom agent's model: only AVAILABLE models are listed
 *  (missing key = hidden), plus the "＋ Add API keys…" settings link. A
 *  selection whose key went missing stays visible — disabled and marked
 *  "(key missing)" — so the closed select never lies about what runs. */
function AgentModelSelect({
  ...
  // "" = the default (main model) option, always present and always valid.
  const stranded = value !== "" && !pickable.some((m) => m.id === value);
  return (
    <select
      id={id}
      value={value}
      onChange={(e) => {
        // The settings link is an action, not a model — the controlled value
        // keeps the previous selection while Settings opens.
        if (e.target.value === ADD_KEYS_VALUE) onAddKeys();
    ...
    >
      ...
      <option value={ADD_KEYS_VALUE}>{ADD_KEYS_LABEL}</option>
    </select>
  );
}
