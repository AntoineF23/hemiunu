...
export function GlobePanel({ open, focusId }: GlobePanelProps) {
  ...
  // refetch the moment the tab/window regains focus. Monuments are earned at
  // most once per published prototype, so a slow poll is plenty — the focus
  // listener covers the "came back to look" case instantly.
  useEffect(() => {
    if (!open) return;
    void load();
