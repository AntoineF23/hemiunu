...
export function TeamsPanel({ open, onChanged }: TeamsPanelProps) {
  ...
  // Hoisted out of the dep array: `data` is state, so `data.current` changing
  // arrives with a re-render and is a valid dependency — but the react-hooks
  // rule sees the `.current` access and mistakes it for a (mutable) ref.
  const currentTeam = data?.current;
  useEffect(() => {
    if (open) void loadTeammates();
  }, [open, currentTeam, loadTeammates]);
