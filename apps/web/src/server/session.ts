...
import { applyPlanDecision, isPlanDecision, type CanUseToolResult } from "@hemiunu/engine";
import type { PermissionDecision, ServerEvent } from "../shared/protocol";

/** What a parked permission resolves to — the engine pipeline's gate answer. */
export type PermissionResult = CanUseToolResult;
...
export function resolvePermission(
  ...
  // A plan-menu choice (exit_plan_mode's gate): the engine maps it to the gate
  // answer — an allow lets exit_plan_mode.execute() flip the mode back to
  // "default"; "plan-refine" denies with the steer message, so the agent stays
  // in plan mode. "plan-auto" also flips this session to auto-accept for the
  // rest of the turn (and the client echoes it back on the next request).
  if (isPlanDecision(decision)) {
    p.resolve(
      applyPlanDecision(decision, {
        setAutoAccept: (on) => {
          s.autoAccept = on;
        },
      }),
