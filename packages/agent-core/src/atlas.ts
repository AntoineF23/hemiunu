...
/** A tier-specific congratulation, shown after the monument name on a new find. */
export const TIER_FLAVOR: Record<Tier, string> = {
  common: "A solid addition to the collection.",
  rare: "A rare one — not every publish turns one up.",
  epic: "An epic find! These don't surface often.",
  legendary: "A legendary monument — a banner day for the atlas.",
  wonder: "A once-in-a-blue-moon find — one of the true Wonders of the World.",
};

/**
 * A deep link into the web app's Atlas, focused on one monument. The map lives
 * in the web app (127.0.0.1, default port 4317); opening this URL in a browser
 * opens the Atlas with the monument selected. Used for the CLI announcement and
 * as the fallback link anywhere a clickable URL is wanted.
 */
export function atlasUrl(id: string): string {
  const port = process.env.HEMIUNU_WEB_PORT ?? "4317";
  return `http://127.0.0.1:${port}/?atlas=${encodeURIComponent(id)}`;
}
...
export function discoveryLine(r: DiscoveryResult): string {
  ...
  if (r.tier === "wonder") {
    return `🌟 WONDER OF THE WORLD! You discovered ${m.name} — ${place}. ${TIER_FLAVOR.wonder} ${progress}`;
  }
  return `${t.emoji} ${t.label} discovery — you unlocked ${m.name}, ${place}. ${TIER_FLAVOR[r.tier]} ${progress}`;
}
