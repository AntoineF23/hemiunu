...
// Per-repo serialization for the git-mutating operations. Two turns bound to the
// SAME repo share one checkout dir (workspacePath), so concurrent fetch / commit
// / reset / clean would race and corrupt the working tree. We serialize them
// through a promise chain keyed by the normalized repo. The lock is REENTRANT
// (via AsyncLocalStorage): a locked op that calls another locked op for the same
// repo — e.g. migrateLocalIntoTeam → ensureWorkspace/commitAndPush — runs inline
// instead of deadlocking on itself.
const repoChains = new Map<string, Promise<unknown>>();
const heldRepos = new AsyncLocalStorage<Set<string>>();

function withRepoLock<T>(repo: string, fn: () => Promise<T>): Promise<T> {
  const key = normalizeRepo(repo);
  const held = heldRepos.getStore();
  if (held?.has(key)) return fn(); // reentrant: already inside this repo's lock
  const prev = repoChains.get(key) ?? Promise.resolve();
  const run = prev
    .catch(() => {}) // a prior failure must not poison the queue
    .then(() => heldRepos.run(new Set([...(held ?? []), key]), fn));
  repoChains.set(
    key,
    run.catch(() => {}),
  );
...
export async function ensureWorkspace(
  ...
  return withRepoLock(repo, async () => {
    ...
    if (dirty) {
      ...
      if (staged)
        await git([...ident, "commit", "-m", "Auto-saved prototype work (pre-sync)"], {
          cwd: path,
        });
    }
...
export async function commitAndPush(
  ...
  return withRepoLock(repo, async () => {
    ...
    if (opts.toMain) {
      ...
      const behind =
        (await gitOut(["rev-list", "--count", `HEAD..origin/${branch}`], path)) !== "0";
      if (behind) {
    ...
    const p = await git(["push", "-u", "origin", `HEAD:${branch}`], {
      cwd: path,
      token: opts.token,
    });
...
export async function renameWorkspace(oldRepo: string, newRepo: string): Promise<void> {
  ...
  try {
    ...
    // renameSync throws EXDEV across filesystems/mount points (e.g. the config
    // dir on a different volume than /tmp). Fall back to copy-then-remove.
