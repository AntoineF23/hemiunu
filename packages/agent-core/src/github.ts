...
function saveGithubAuth(f: GithubAuthFile): void {
  ...
  invalidateGhCliToken(); // auth changed — never serve a stale fallback token
}
 ...
 */
// The `gh auth token` fallback shells out to a subprocess, and resolveGithubToken
// is called on hot per-tool paths (resolveRepo). Cache that subprocess result
// briefly; the store/env paths stay uncached (cheap reads, and they must react
// to connect/disconnect immediately).
const GH_CLI_TOKEN_TTL_MS = 300_000;
let ghCliToken: { value: string | undefined; at: number } | null = null;
/** Drop the cached `gh` CLI token (call on connect/disconnect/switch). */
function invalidateGhCliToken(): void {
  ghCliToken = null;
}
...
export function resolveGithubToken(): string | undefined {
  ...
  if (env) return env;
  if (ghCliToken && Date.now() - ghCliToken.at < GH_CLI_TOKEN_TTL_MS) return ghCliToken.value;
