...
- **MCP servers are cwd-sandboxed.** The SDK gives stdio servers no per-server `cwd`, so they'd inherit the launch dir and write artifacts into the user's project (Playwright dropped `.playwright-mcp/`). `sandboxStdioCwd` (`packages/mcp/config.ts`) rewrites each stdio server to launch via `bin/mcp-in-dir.mjs`, which chdir's into `~/.hemiunu/tmp/mcp/<name>` before exec'ing the real command (stdio passes straight through). The **filesystem** server is the sole exemption (reading the launch project is its job). Applied at both registry-load sites (CLI `index.tsx`, web `runtime.ts`); `build-release.mjs` ships the shim. The agent's own write tools already target `~/.hemiunu/tmp` or the team repo — never the launch dir.
