...
6. **Prototyping layer** — a **two-stage** flow:
   ...
   > **BUILT (Slice 3a, 2026-06-22) — Stage A wireframes, first slice.** The pipeline is **research → brief → wireframe → preview → iterate**, with a structured brief (goal · primary user + JTBD · entities · screens/sections/components · real content) as the grounding intermediate: the `researcher` fills brief slots from the sources, the **`prototyper`** subagent (on the synthesis tier) turns the brief into a **self-contained low-fi HTML wireframe** (grayscale, real labels, inline CSS, no external requests) and saves it via the new **`save_prototype`** tool (`packages/agent-core/src/prototype.ts`): writes under `prototypes/<slug>/` with a path-traversal guard, then opens `index.html` in the browser (suppressed by `HEMIUNU_NO_OPEN`). Exported pure `savePrototype()` is smoke-tested (sandbox + traversal). CLI shows `⌂ prototyper · <model>` and a "Prototyping" status. soul.md carries the two-step flow. **Decision: low-fi = HTML** (fastest iteration; design system enters at Stage B, not here). **Architectural finding:** a subagent's tools do NOT need to be in the parent `tools` allowlist (registration + `AgentDefinition.tools` suffices, verified via `allowedTools`) — so a later refinement can hide `save_prototype` from the main loop to *force* delegation (pending verification under the `canUseTool` path). **Not yet:** multi-screen + flows, design-system grounding (Stage B), screenshot self-critique, Vercel deploy.
   >
   > **Knowledge layer (2026-06-22):** domain guidelines live in `context/knowledge/<topic>.md` (tracked, not per-user). First entry: `design.md` (Apple's 8 principles of great design). `subagents.ts` `subagentPrompt(name)` injects the relevant doc into a subagent's system prompt — the `prototyper` carries the full design guideline (with a note that visual Craft/Delight apply at hi-fi, not the low-fi wireframe stage); other subagents don't. A compact 8-principle digest lives in `soul.md` so the MAIN loop gives grounded design *advice* without paying the full doc's tokens every turn. This is the pattern for "knowledge for every part of the product" — add `eng.md`, `data.md`, etc. and wire them to the relevant subagent/skill.
...
>
> **BUILT (Slice 2, 2026-06-22):** the **`ask_model`** tool — non-Claude models as tools. An in-process SDK MCP server (`hemiunu-models`) exposes `ask_model({ model, prompt, system?, max_tokens? })`, a one-shot call via the proxy's **OpenAI `/v1/chat/completions`** endpoint (the reliable path for non-Claude models; same bearer key). Always available + permission-gated; the Claude main loop stays the brain and integrates the result. Core extracted as an exported `askModel()` for a deterministic smoke check (calls `gemini-2.5-flash`). soul.md lists valid proxy ids. Reasoning models need headroom — default `max_tokens` 2000 (a tiny budget returns empty content with `finish_reason: length`).
>
> **UPDATED (2026-06-22) — bring-your-own providers.** Decoupled from the org proxy. The **brain** now defaults to **Anthropic direct** (`ANTHROPIC_BASE_URL` optional — set it for a gateway/proxy). **`ask_model` is provider-aware** (`packages/agent-core/src/providers.ts`): `ask_model({provider, model, …})` where provider ∈ `openai|google|groq|xai|deepseek|mistral|proxy`, each an OpenAI-compatible endpoint resolving its key from a per-provider env var (`OPENAI_API_KEY`, `GEMINI_API_KEY`, …) with an optional `<PROVIDER>_BASE_URL` override. `proxy` reuses `ANTHROPIC_BASE_URL`+`ANTHROPIC_API_KEY` (for gateway users). Missing key → a helpful "add `X_API_KEY`" message with NO network call. So users bring their own keys per provider; the brain needs only an Anthropic key.
>
> **BUILT (Slice 3b, 2026-06-22) — parallel execution.** Subagents give context isolation (each runs in its own window; only its result returns), but **the models will NOT fan out `Task` calls themselves** — verified: both Sonnet and Opus dispatch subagents one-per-turn even when explicitly told to parallelise, so model-driven parallelism is a dead end here. Parallelism is therefore done in **deterministic code**: `subagents.ts` centralises the subagent specs (prompt/tier/tools) as the single source for both the SDK `agents` map and an exported `runSubagent(name, prompt, ctx)` that runs one isolated subagent turn to completion; `orchestrator.ts` exposes a **`parallel`** tool (`hemiunu-orchestrator`) that takes `tasks: [{agent, prompt, label?}]` and runs them through a bounded `pool()` (cap 5, order-preserving) concurrently, returning merged labelled results. The fan-out is real (verified: 3 researcher runs completed in ~1× the slowest task's wall-clock, not 3×). Sub-runs auto-approve their already-scoped tools — the gate is approving the `parallel` call. Recursion is prevented by not giving sub-runs the orchestrator/Task tools. soul.md steers it; the CLI shows `⌂ parallel · N tasks · labels` + an "Orchestrating" status; `pool()` has a deterministic offline smoke check. **Limitation:** sub-run inner tool calls don't stream to the CLI (they're internal to the tool), so parallel work is less granular on screen than a single `Task` delegation — acceptable for now.
...
- **Non-Claude models are exposed as tools, not as the brain (BUILT):** the `ask_model({ model, prompt, system?, max_tokens? })` tool calls other providers **through the LiteLLM proxy** (`models.thiga.co`, OpenAI chat-completions endpoint) for specific subtasks; the Claude-driven agent decides when to call them. This gives multi-provider reach without destabilizing the agent loop.
...
**Phase 4 — Subagents & governance.** Define `researcher` + `prototyper` subagents (model-specialized); add RBAC enforced via `canUseTool` + role→tool/MCP policy; add `PostToolUse` audit hook and `maxBudgetUsd`. *Done when:* a low-rights user is blocked from a restricted tool, with an audit entry. **Partial:** the `researcher` subagent + model tiers AND the `prototyper` subagent (Stage A wireframes — see BUILT notes under Model strategy and Prototyping) are built; RBAC, audit hooks, and budget caps remain.
