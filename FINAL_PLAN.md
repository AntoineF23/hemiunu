...
> **BUILT (2026-06-26) — Stage B hi-fi handoff.** A dedicated **`designer`** subagent (synthesis tier, `packages/agent-core/src/subagents.ts`) upgrades an approved wireframe (or a clear brief) to a high-fidelity prototype. It reads the wireframe from the workspace, then builds against **whichever design-system MCP is connected** (its tools come through the same `...sourceTools` wiring the researcher uses) — recreating the DS's components/tokens/fonts exactly. With **no DS connected** it falls back to a real multi-file **Vite + React + TypeScript + Tailwind v4** project, served by `preview.ts`'s existing dev-server path (`npm install` + `npm run dev`). Knowledge pack: `context/knowledge/hifi-design.md` (DS-first discipline, the fallback stack + quality bar, tokens/states/a11y/motion). soul.md carries the three-stage flow (wireframe → hi-fi) and tells the `designer` which DS is connected. **Decisions:** separate subagent (not a `prototyper` mode); real Vite project, not a single-file CDN page. **Not yet:** Vercel Sandbox run/test and shared-preview deploy.
...
5. **Tools, skills & subagents** — in-process custom tools via `tool()` + `createSdkMcpServer({ type: 'sdk' })`; **Agent Skills** in `~/.hemiunu/skills/`; specialized **subagents** (`researcher`, `prototyper`, `designer`, and planned `spec-writer`/`evaluator`) via the `agents` option.
...
**Phase 5 — The prototyping pipeline (three stages).**
  ...
  - *5b — Hi-fi via design system:* **BUILT (2026-06-26)** — on approval, the `designer` subagent builds against **whichever design-system MCP the team has connected** (Figma / shadcn / DIVE / a custom registry) to produce the near-production prototype, falling back to a real Vite + React + TS + Tailwind v4 project when none is connected. *Done when:* an approved wireframe is upgraded to a styled prototype using real DS components. **Still to do:** deploy + track alongside its wireframe lineage in Vercel Sandbox / the shared Vercel project.
